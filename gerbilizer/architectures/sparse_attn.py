...
def pad_mat(mat: torch.Tensor, block_size: int):
    ...
    """
    nearest_mult_d1 = (
        mat.shape[-2] + block_size - (mat.shape[-2] % block_size)
        if (mat.shape[-2] % block_size != 0)
        else mat.shape[-2]
    )
    nearest_mult_d2 = (
        mat.shape[-1] + block_size - (mat.shape[-1] % block_size)
        if (mat.shape[-1] % block_size != 0)
        else mat.shape[-1]
    )
    padded = torch.zeros(
        (*mat.shape[:-2], nearest_mult_d1, nearest_mult_d2),
        dtype=mat.dtype,
        device=mat.device,
    )
...
def sparse_attn(
    ...
    *,
    ...
    if q.dim() == 3:
        k, q, v = (
            a.unsqueeze(1) for a in (k, q, v)
        )  # unsqueeze in attn head dimension

    if any(mat.dim() != 4 for mat in (q, k, v)):
        raise ValueError(
            "Query, Key, and Value matrices are expected to have shape (batch, heads, seq_len, d_model) or (batch, seq_len, d_model) or (heads, seq_len, d_model"
        )
    ...
    if n_global < 0 or n_window < 0 or n_random < 0:
        raise ValueError(
            "Attention parameters n_global, n_window, and n_random should be positive integers"
        )
    ...
    if q.shape[3] != k.shape[3] or q.shape[3] != v.shape[3]:
        raise ValueError(
            "Query, key, and Value sequence elements should contain the same number of features."
        )
    ...
    q_toprow, q_sparse = torch.split(
        q, [n_global * block_size, q_len - n_global * block_size], dim=2
    )
    ...
    k_block_global, k_block_sparse = torch.split(
        k, [n_global * block_size, q_len - n_global * block_size], dim=2
    )
    k_block_global = k_block_global.reshape(
        batch_size, n_heads, -1, block_size, d_model
    )
    k_block_sparse = k_block_sparse.reshape(
        batch_size, n_heads, -1, block_size, d_model
    )
    ...
    global_col = k_block_global.view(
        batch_size, n_heads, 1, n_global * block_size, d_model
    )
    global_col = global_col.expand(-1, -1, n_blocks - n_global, -1, -1)
    global_col_attn = torch.einsum(
        "...IJ,...KJ->...IK", q_sparse, global_col
    )  # (batch, heads, blocks - n_global, block_size, n_global * block_size)
    global_col_attn = global_col_attn.view(
        batch_size, n_heads, -1, n_global * block_size
    )  # (batch, heads, num_nonglobal_elements, num_global_elements)
    if num_pad > 0:
    ...
    windowed = torch.cat(
        [
            torch.roll(k_block_sparse, shift, dims=-3)
            for shift in range(-win_radius, win_radius + 1)[::-1]
        ],
        dim=-2,
    )
    ...
    windowed_attn = windowed_attn.view(
        batch_size, n_heads, q_len - block_size * n_global, n_window * block_size
    )
    ...
    if num_pad > 0:
        ...
        for row_idx in range(ng_blocks - win_radius - 1, ng_blocks):
            ...
            illegal_start_idx = (
                win_center + block_idx * block_size + (block_size - num_pad)
            )
        ...
        for row_idx in range(win_radius):
            ...
            good_start_idx = (
                num_illegal_blocks * block_size
            )  # Everything after this point is OK
            windowed_attn_mask[
                ..., row_idx * block_size : (row_idx + 1) * block_size, :good_start_idx
            ] = mask_value
    ...
    if rand_idx is None:
        ...
        win_start = (
            lambda row: row - win_radius
        )  # index of the first key block in the sliding window
        win_end = (
            lambda row: row + win_radius
        )  # index of the last key block in the window
        # Valid indices from which random blocks may be selected given a row (relative to the end of the global rows)
        rand_valid_idx = lambda row: np.array(
            [
                a
                for a in range(n_blocks - n_global)
                if a < win_start(row) or a > win_end(row)
            ]
        )
        ...
        rand_gather_idx = torch.zeros(
            (1, 1, k_block_sparse.shape[-3] * n_random, 1, 1),
            device=k_block_sparse.device,
            dtype=torch.int64,
        )
        for row, valid_idx in enumerate(rand_sampled_cols):
            rand_gather_idx[
                0, 0, row * n_random : (row + 1) * n_random, 0, 0
            ] = torch.from_numpy(valid_idx)
        # rand_gather_idx = rand_gather_idx.expand((1, n_heads, -1, block_size, d_model))
    else:
        rand_gather_idx = rand_idx
        rand_requires_masking = [
            rand_gather_idx[0, 0, (row + 1) * n_random - 1, 0, 0]
            == (n_blocks - n_global - 1)
            for row in range(n_blocks - n_global)
        ]
    ...
    rand_cols = torch.gather(
        k_block_sparse,
        dim=2,
        index=rand_gather_idx.expand((batch_size, n_heads, -1, block_size, d_model)),
    )
    rand_cols = rand_cols.view(
        batch_size, n_heads, k_block_sparse.shape[-3], n_random * block_size, d_model
    )
    rand_attn = torch.einsum(
        "...IJ,...KJ->...IK", q_sparse, rand_cols
    )  # Should have shape (batch_size, n_heads, non_global_blocks, block_size, n_random*block_size)
    ...
    top_row_scores = torch.softmax(
        top_row_attn, dim=-1
    )  # These contain every column already, can use normal softmax
    ...
    # Calc. denominator
    denominator = (
        global_col_scores.sum(dim=-1, keepdims=True)
        + windowed_scores.sum(dim=-1, keepdims=True)
        + rand_scores.sum(dim=-1, keepdims=True)
    )
    ...
    global_col_vals, v_sparse = torch.split(
        v, [n_global * block_size, v_len - n_global * block_size], dim=2
    )
    # (batch, heads, n_nonglobal_elements, n_global_elements) x (batch, heads, n_global_elements, d_model)
    global_col_output = torch.einsum(
        "...IJ,...JK->...IK", global_col_scores, global_col_vals
    )
    ...
    windowed_values = torch.cat(
        [
            torch.roll(v_block_sparse, shift, dims=-3)
            for shift in range(-win_radius, win_radius + 1)[::-1]
        ],
        dim=-2,
    )
    ...
    windowed_scores = windowed_scores.view(
        batch_size, n_heads, -1, block_size, block_size * n_window
    )
    # Each score needs to be broadcast out 'block_size' times, since they all share the same value window
    windowed_output = torch.einsum(
        "...IJ,...JK->...IK", windowed_scores, windowed_values
    ).view(batch_size, n_heads, -1, d_model)
    ...
    v_rand = v_rand.view(
        batch_size, n_heads, v_block_sparse.shape[-3], n_random * block_size, d_model
    )
    # Same thing that happened with the windows happens here
    rand_scores = rand_scores.view(
        batch_size, n_heads, -1, block_size, n_random * block_size
    )
    rand_output = torch.einsum("...IJ,...JK->...IK", rand_scores, v_rand).view(
        batch_size, n_heads, -1, d_model
    )
    ...
    sparse_output = (
        pad_sparse_output if num_pad == 0 else pad_sparse_output[..., :-num_pad, :]
    )
